...
for percentage in percentages:
    for ratio, ratio_text in zip(ratios, ratios_text):  # Iterate over train-test split ratios
        ...
        df = pd.read_csv("Pipeline/stats_raytrace_Pipeline_16/Data-traffic-distribution-Comparison.csv")
        ...
        for index in tqdm(np.unique(df[class_column])):
            ...
            tmp_out.append([f'Target Class: {labels[index]}', 'TP', 'FN', 'FP', 'TN', 'ACC (%)', 'Noise Percantages', 'Upsampling', ''])  # TP, FP, etc. are placeholders for output
        ...
        out.to_csv("Pipeline/stats_raytrace_Pipeline_16/Data-traffic-distribution-Results.csv", header=False, index=False, mode='a')  # Save the result